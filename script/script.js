...
class Dice {
    ...
    constructor(size = 100, originCorX = 100 + size, originCorY = 100 + size) {
        ...
        this._square.center = { x: (this._square.point1.x + this._square.point3.x) / 2, y: (this._square.point1.y + this._square.point3.y) / 2 };
    ...
    searchCord(numPoint) {
        let pointCircle = {
            ...
            3: { x: (((this._square.center.x + this._square.point1.x) / 2) + ((this._square.center.x + this._square.point2.x) / 2)) / 2, y: (((this._square.center.y + this._square.point1.y) / 2) + ((this._square.center.y + this._square.point2.y) / 2)) / 2 },
            ...
            6: { x: (((this._square.center.x + this._square.point4.x) / 2) + ((this._square.center.x + this._square.point3.x) / 2)) / 2, y: (((this._square.center.y + this._square.point4.y) / 2) + ((this._square.center.y + this._square.point3.y) / 2)) / 2 },
        ...
        switch (numPoint) {
            ...
            case 5:
                return deepClone({}, { 1: pointCircle['1'], 2: pointCircle['2'], 4: pointCircle['4'], 5: pointCircle['5'], 7: pointCircle['7'] });
            case 6:
                return deepClone({}, { 2: pointCircle['2'], 3: pointCircle['3'], 4: pointCircle['4'], 5: pointCircle['5'], 6: pointCircle['6'], 7: pointCircle['7'] });
...
class GameDices {
    ...
    constructor(number = 6, size = 100, objCanvas) {
        ...
        for (let i = 0; i < number; i++) {
            let tempDice;
            try {
                tempDice = new Dice(size, (Math.random() * (this.canvas.width - 3 * size) + 1.5 * size), (Math.random() * (this.canvas.height - 3 * size) + 1.5 * size));
    ...
    isIntersections(pointsObj1, pointsObj2) {
        ...
        for (const iterator of arguments) {
            ...
            name.push([Object.values(temp[0]), Object.values(temp[1])], [Object.values(temp[1]), Object.values(temp[2])], [Object.values(temp[2]), Object.values(temp[3])], [Object.values(temp[3]), Object.values(temp[0])]);
            name = secondSquareLines;
        }
        for (const iterator1 of firstSquareLines) {
            for (const iterator2 of secondSquareLines) {
                let v1 = (iterator2[1][0] - iterator2[0][0]) * (iterator1[0][1] - iterator2[0][1]) -
                    (iterator2[1][1] - iterator2[0][1]) * (iterator1[0][0] - iterator2[0][0]);
                let v2 = (iterator2[1][0] - iterator2[0][0]) * (iterator1[1][1] - iterator2[0][1]) -
                    (iterator2[1][1] - iterator2[0][1]) * (iterator1[1][0] - iterator2[0][0]);
                let v3 = (iterator1[1][0] - iterator1[0][0]) * (iterator2[0][1] - iterator1[0][1]) -
                    (iterator1[1][1] - iterator1[0][1]) * (iterator2[0][0] - iterator1[0][0]);
                let v4 = (iterator1[1][0] - iterator1[0][0]) * (iterator2[1][1] - iterator1[0][1]) -
...
function draw() {
    try {
        ...
        ctx.setAttribute('width', size.width * 0.9);
        ctx.setAttribute('height', size.height * 0.9);
